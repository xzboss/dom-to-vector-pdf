...
2. 字体包可以裁剪，一般包含常用字都能裁剪到1MB以内。**⚠️⚠️⚠️这里很多在线裁剪的库裁剪完后不会包含空格**，导致渲染失败。这个最好用库来裁剪。常用汉字字集可以参考[汉字字集](./font-subset)页面。
3. 导出速度受字体和网速影响，需要加在完资源才会做导出，字体越大，越多速度越慢，pdf产物也会越大。
